...
# Interactive sessions do NOT inherit the image/application environment: CF's
# diego-sshd hands `cf ssh` a minimal PATH (/bin:/usr/bin) and drops the app
# env, and `railway ssh` / a plain `ssh` into the container behaves the same.
# Without this, `hermes`/`s6` were not on PATH and HERMES_HOME was unset, so a
# CLI run in such a session silently used ~/.hermes instead of the deployed
# state dir (a second, ephemeral home). This runs BEFORE the /init hand-off so
# both the PID-1 and the fallback layout get it. Published to every place a
